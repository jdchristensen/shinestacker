...
* ```channel``` (optional, default: BALANCE_LUMI): channels to be balanced. Possible values are: 
   ...
   - ```BALANCE_HLS```: balance saturation and luminosity value in the HLS (Hue, Lightness, Saturation) color space, it may be needed in cases of extreme luminosity variation that affects saturation (8 bit only);
   - ```BALANCE_LAB```: balance luminosity in the LAB color space (8 bit only).
