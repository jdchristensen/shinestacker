...
def test_undo_manager_initialization():
    ...
    undo_manager = UndoManager(mock_transformation_manager)
    assert undo_manager.undo_stack == []
    assert undo_manager.redo_stack == []
    assert undo_manager.x_start == gui_constants.MAX_UNDO_SIZE
    assert undo_manager.y_start == gui_constants.MAX_UNDO_SIZE
    assert undo_manager.x_end == 0
    assert undo_manager.y_end == 0
...
def test_reset():
    ...
    undo_manager.x_start = 10
    undo_manager.y_start = 20
    undo_manager.x_end = 30
    undo_manager.y_end = 40
    undo_manager.reset()
    assert undo_manager.undo_stack == []
    assert undo_manager.redo_stack == []
    assert undo_manager.x_start == gui_constants.MAX_UNDO_SIZE
...
def test_extend_undo_area():
    mock_transformation_manager = Mock()
    undo_manager = UndoManager(mock_transformation_manager)
    undo_manager.x_start = 5
    undo_manager.y_start = 5
    undo_manager.x_end = 10
    undo_manager.y_end = 10
    undo_manager.extend_undo_area(3, 3, 12, 12)
    assert undo_manager.x_start == 3
    assert undo_manager.y_start == 3
    assert undo_manager.x_end == 12
    assert undo_manager.y_end == 12
...
def test_undo_rotate_operations():
    ...
    undo_manager = UndoManager(mock_transformation_manager)
    mock_layer = np.zeros((20, 20))
    undo_manager.undo_stack = [{'description': gui_constants.ROTATE_90_CW_LABEL, 'area': (0, 0, 0, 0)}]
    undo_manager.undo(mock_layer)
    mock_transformation_manager.rotate_90_ccw.assert_called_once_with(False)
    mock_transformation_manager.reset_mock()
    undo_manager.undo_stack = [{'description': gui_constants.ROTATE_90_CCW_LABEL, 'area': (0, 0, 0, 0)}]
    undo_manager.undo(mock_layer)
    mock_transformation_manager.rotate_90_cw.assert_called_once_with(False)
    mock_transformation_manager.reset_mock()
    undo_manager.undo_stack = [{'description': gui_constants.ROTATE_180_LABEL, 'area': (0, 0, 0, 0)}]
    undo_manager.undo(mock_layer)
...
def test_redo_rotate_operations():
    ...
    undo_manager = UndoManager(mock_transformation_manager)
    mock_layer = np.zeros((20, 20))
    undo_manager.redo_stack = [{'description': gui_constants.ROTATE_90_CW_LABEL, 'area': (0, 0, 0, 0)}]
    undo_manager.redo(mock_layer)
    mock_transformation_manager.rotate_90_cw.assert_called_once_with(False)
    mock_transformation_manager.reset_mock()
    undo_manager.redo_stack = [{'description': gui_constants.ROTATE_90_CCW_LABEL, 'area': (0, 0, 0, 0)}]
    undo_manager.redo(mock_layer)
    mock_transformation_manager.rotate_90_ccw.assert_called_once_with(False)
    mock_transformation_manager.reset_mock()
    undo_manager.redo_stack = [{'description': gui_constants.ROTATE_180_LABEL, 'area': (0, 0, 0, 0)}]
    undo_manager.redo(mock_layer)
    mock_transformation_manager.rotate_180.assert_called_once_with(False)
